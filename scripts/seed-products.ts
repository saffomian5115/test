/**
 * Seed Script — 25 dummy products daalne ke liye.
 * 
 * Chalao: npx tsx scripts/seed-products.ts
 * (Pehle MongoDB URI .env.local me set karna na bhoolein!)
 */
...
if (!MONGODB_URI) {
  console.error("❌ MONGODB_URI .env.local me set nahi hai!");
  process.exit(1);
}
...
async function seed() {
  try {
    ...
    // Product schema/model — local copy to avoid import issues
    ...
    // Purani products delete karo aur naye daalo
    await Product.deleteMany({});
    console.log("🗑️ Purane products delete!");

    const inserted = await Product.insertMany(products);
    console.log(`✅ ${inserted.length} products successfully added!`);
